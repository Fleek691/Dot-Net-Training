...
public class Portfolio<T> where T : IFinancialInstrument
{
    ...
    public void Buy(T instrument, int quantity, decimal price)
    {
        ...
        if (_holdings.ContainsKey(instrument))
        {
            ...
            System.Console.WriteLine("Updated qty");
            return;
        }
        _holdings[instrument] = quantity;
    ...
    public decimal? Sell(T instrument, int quantity, decimal currentPrice)
    {
        ...
        {
            System.Console.WriteLine("Insufficient");
            return 0;
        }
        _holdings[instrument] -= quantity;
        ...
        return instrument.CurrentPrice * quantity;
    ...
    public decimal CalculateTotalValue()
    {
        ...
        var total = _holdings.Sum(e => e.Key.CurrentPrice * e.Value);
        return total;
    }
    ...
    {
        // Only check instruments in current holdings
        if (_holdings.Count == 0)
            return null;

        var bestPerformer = _holdings
            .Where(kvp => purchasePrices.ContainsKey(kvp.Key))
            .Select(kvp => new
            {
                instrument = kvp.Key,
                returnPercentage = ((kvp.Key.CurrentPrice - purchasePrices[kvp.Key])
                    / purchasePrices[kvp.Key]) * 100
            })
        ...
        return (bestPerformer.instrument, bestPerformer.returnPercentage);
    }
}
